# FROM https://github.com/taoshen58/LexMAE
...
def generate_bottleneck_repre(
    ...
    if bottleneck_src == "cls":
        ...
        if bottleneck_src == "logits":
            ...
            pooled_enc_saturated_logits = torch.log(
                torch.relu(pooled_enc_logits) + 1.0
            )  # bs,V
        ...
        bottleneck_repre = torch.matmul(
            pooled_enc_probs, word_embeddings_matrix.detach()
        )  # bs,h
    else:
        raise NotImplementedError(bottleneck_src)
...
class RobertaForEDMLM(RobertaPreTrainedModel):
    _keys_to_ignore_on_save = [
        ...
        r"decoder_lm_head.decoder.weight",
        r"decoder_lm_head.decoder.bias",
    ]
    ...
    def tie_weights(self):
        super().tie_weights()
        self._tie_or_clone_weights(self.decoder_lm_head.decoder, self.lm_head.decoder)
    ...
    def forward_decoder_heads(
        ...
        dec_inputs_embeds=None,
        dec_past_key_values_length=0,
        ...
        if dec_input_ids is not None:
            ...
            if dec_token_type_ids is None:
                if hasattr(self.roberta.embeddings, "token_type_ids"):
                    buffered_token_type_ids = self.roberta.embeddings.token_type_ids[
                        :, :seq_length
                    ]
                    ...
                    dec_token_type_ids = torch.zeros(
        ...
        **kwargs,
        ...
        if not disable_encoding:
            ...
            enc_cls_rep = generate_bottleneck_repre(
                ...
                last_hidden_states=enc_hidden_states[-1],
                mlm_logits=return_dict.logits,
            )
...
class BertForEDMLM(BertPreTrainedModel):
    ...
    def tie_weights(self):
        ...
        self._tie_or_clone_weights(
            self.decoder_cls.predictions.decoder, self.cls.predictions.decoder
        )
...
class DistilBertForEDMLM(DistilBertPreTrainedModel):
    ...
    def decoder_cls_head(self, hidden_states):
        prediction_logits = self.decoder_vocab_transform(
            hidden_states
        )  # (bs, seq_length, dim)
        prediction_logits = gelu(prediction_logits)  # (bs, seq_length, dim)
        prediction_logits = self.decoder_vocab_layer_norm(
            prediction_logits
        )  # (bs, seq_length, dim)
        prediction_logits = self.decoder_vocab_projector(
            prediction_logits
        )  # (bs, seq_length, vocab_size)
        return prediction_logits
    ...
    def forward_decoder_heads(
        ...
        if dec_input_ids is not None:
            dec_embeddings = self.distilbert.embeddings(
                input_ids=dec_input_ids,
            )
        else:
            assert enc_hidden_states is not None and self.skip_from is not None
    ...
    def forward(
        ...
        if not disable_encoding:
            ...
            enc_cls_rep = generate_bottleneck_repre(
                ...
                "logits",
                self.special_token_ids,
                self.distilbert.embeddings.word_embeddings.weight,
                last_hidden_states=enc_hidden_states[-1],
                mlm_logits=return_dict.logits,
            )
            return_dict["sentence_embedding"] = enc_cls_rep
