        ...
        **kwargs,
        ...
        if dec_masked_input_ids is None:
            if self.cfg.dec_mlm_overlap == "random":
                ...
                assert self.cfg.dec_mlm_prob >= self.cfg.enc_mlm_prob
                extra_masked_flags, exclude_flags = masked_flags, None
                dec_mlm_prob = (self.cfg.dec_mlm_prob - self.cfg.enc_mlm_prob) / (
                    1.0 - self.cfg.enc_mlm_prob
                )
            elif self.cfg.dec_mlm_overlap == "exclusive":
                assert self.cfg.dec_mlm_prob <= (1.0 - self.cfg.enc_mlm_prob)
                extra_masked_flags, exclude_flags = None, masked_flags
                dec_mlm_prob = self.cfg.dec_mlm_prob / (1.0 - self.cfg.enc_mlm_prob)
            else:
        ...
        loss_dict["loss"] = (
            ...
            + self.cfg.mlm_dec_loss_weight * loss_dict["mlm_dec_loss"]
            + self.cfg.bow_loss_weight * loss_dict["bow_loss"]
        )
        return loss_dict
...
def main(cfg: DictConfig):
    ...
    train_dataloader = DataLoader(
        ...
        num_workers=4,
        batch_size=cfg.batch_size,
        collate_fn=LexMAECollateDupMAE(
            tokenizer, max_length=cfg.model.max_length, vocab_size=config.vocab_size
        ),
        pin_memory=True,
    )
